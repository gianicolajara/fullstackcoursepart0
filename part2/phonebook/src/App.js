...
const App = () => {
  ...
  const handleDeletePerson = (id) => {
    ...
    personsServices
      ...
      .catch((err) => {
        setErrorMsg(
          err.message || `Person with id ${id} was already removed server`,
        )
  ...
  const handleUpdatePerson = (id) => {
    ...
    personsServices
      ...
      .catch((err) => {
        setErrorMsg(err.message || `${personToUpdate.name} cannt be updated`)
        setTimeout(() => setErrorMsg(null), 5000)
      })
  }
  ...
  const handleCreaterPerson = () => {
    ...
    personsServices
      ...
      .then((res) => {
        ...
        setSuccessMsg(`${newPerson.name} has been created`)
        setTimeout(() => setSuccessMsg(null), 5000)
      })
      .catch((err) => {
        setErrorMsg(err.message || `${newPerson} had a problem when created`)
        setTimeout(() => setErrorMsg(null), 5000)
      })
  }
  ...
  useEffect(() => {
    personsServices
      ...
      .catch((err) => setErrorMsg(err.message || `problems bringing a person`))
  }, [])
